...
void sharpenNPP(Npp8u *arr, int w, int h) {
	...
	nppiFilterBorder32f_8u_C3R(d_in, kNumCh*w*h * sizeof(Npp8u), oSrcSize, oSrcOffset, d_out, 
		kNumCh*w * sizeof(Npp8u), oSizeROI, d_filter, oKernelSize, oAnchor, NPP_BORDER_REPLICATE);
...
int main() {
	cimg_library::CImg<unsigned char> image("39.bmp");
	...
	for (int r = 0; r < h; ++r) {
		for (int c = 0; c < w; ++c) {
			for (int ch = 0; ch < kNumCh; ch++) {	
				dif += image(c, r, ch) - arr[kNumCh*(r*w + c) + ch];
